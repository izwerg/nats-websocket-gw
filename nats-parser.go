...
func (c *client) parse(buf []byte) error {
	...
	for i = 0; i < len(buf); i++ {
		...
		case OP_START:
			...
			//	c.state = OP_MINUS
			default:
				fmt.Println("ERROR: OP_START", b, i, len(buf), string(buf))
				goto parseErr
			}
		case OP_P:
			switch b {
	...
	if c.state == SUB_ARG || c.state == UNSUB_ARG || c.state == PUB_ARG ||
		...
		//if len(c.argBuf) > mcl {
		if len(c.argBuf) > MAX_CONTROL_LINE_SIZE {
			c.sendErr("Maximum Control Line Exceeded")
			//c.closeConnection() // TODO: CLOSE CONNECTION
...
parseErr:
	//c.sendErr("Unknown Protocol Operation") // TODO
	snip := protoSnippet(i, buf)
	//err := fmt.Errorf("%s Parser ERROR, state=%d, i=%d: proto='%s...'", c.typeString(), c.state, i, snip)
	err := fmt.Errorf("%s Parser ERROR, state=%d, i=%d: proto='%s...'", "Client", c.state, i, snip)
	return err
}
